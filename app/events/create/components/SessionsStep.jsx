...
export default function SessionsStep() {
  ...
  const isSessionComplete = (session) => {
    if (!session?.title?.trim() || !session?.startTime || !session?.endTime) {
      return false;
    }
  ...
  const handleAddSession = () => {
    const sessionsList = eventDraft.sessions;
    const lastSessionIndex = sessionsList.length - 1;

    if (
      lastSessionIndex >= 0 &&
      !isSessionComplete(sessionsList[lastSessionIndex])
    ) {
      setExpandedSession(lastSessionIndex);
      toast.error(
        "Complete the current session details before adding another one",
      );
      return;
    }

    sessions.add();
    setExpandedSession(sessionsList.length);
  };

  const addSpeaker = (sessionIndex) => {
    const session = eventDraft.sessions[sessionIndex];
    const speakerName = speakerInputs[sessionIndex]?.trim();
    ...
    const existingSpeakers = session.speakers || [];
    if (
      existingSpeakers.some(
        (speaker) => speaker.toLowerCase() === speakerName.toLowerCase(),
      )
    ) {
    ...
    sessions.update(sessionIndex, {
      speakers: [...existingSpeakers, speakerName],
    });

    setSpeakerInputs((prev) => ({ ...prev, [sessionIndex]: "" }));
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Add sessions and schedule for your event
          </p>
        </div>
                ...
                <CardContent className="space-y-4 pt-0">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={session.title || ""}
                      onChange={(e) =>
                        sessions.update(index, { title: e.target.value })
                      }
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        sessions.update(index, { description: e.target.value })
                      }
                      rows={3}
                    />
                  </div>
                    ...
                    <div className="border-t pt-4 space-y-4">
                      <h4 className="font-semibold flex items-center gap-2">
                        ...
                        <Badge variant="outline">
                          Required for {eventDraft.mode} events
                        </Badge>
                      </h4>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Enable participant check-in
                        </p>
                      </div>
                      <Switch
                        checked={session.allowCheckIn ?? true}
                        onCheckedChange={(checked) =>
                          sessions.update(index, { allowCheckIn: checked })
                        }
                      />
                    </div>
                          ...
                          <DateTimePicker
                            value={
                              session.checkInStartTime
                                ? new Date(session.checkInStartTime)
                                : null
                            }
                            onChange={(date) =>
                              sessions.update(index, {
                                checkInStartTime: date?.toISOString(),
                              })
                            }
                            use12HourFormat={true}
                          />
                          ...
                          <DateTimePicker
                            value={
                              session.checkInEndTime
                                ? new Date(session.checkInEndTime)
                                : null
                            }
                            onChange={(date) =>
                              sessions.update(index, {
                                checkInEndTime: date?.toISOString(),
                              })
                            }
                            use12HourFormat={true}
                          />
                  ...
                  <div className="flex items-center justify-between border-t pt-4">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Make this session available
                      </p>
                    </div>
                    <Switch
                      checked={session.isActive ?? true}
                      onCheckedChange={(checked) =>
                        sessions.update(index, { isActive: checked })
                      }
                    />
                  </div>
                </CardContent>
