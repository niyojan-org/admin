...
export default function CreateEventPage() {
  ...
  const validateStep = (stepId, showToast = false) => {
    ...
    if (stepId === 0) {
      ...
      if (
        ...
        new Date(eventDraft.registrationStart) >= new Date(eventDraft.registrationEnd)
      ) {
    ...
    if (stepId === 1) {
      ...
      eventDraft.sessions.forEach((session, index) => {
        ...
        if (isVenueRequired && !session.venue?.name?.trim()) {
          errors.push(`Session ${index + 1}: venue name is required for ${eventDraft.mode} events`);
        }
      });
    }
      ...
      eventDraft.tickets.forEach((ticket, index) => {
        ...
        if (ticket.isGroupTicket) {
          ...
          if (minParticipants < 2) {
            errors.push(`Ticket ${index + 1}: minimum group participants must be at least 2`);
          }
          if (maxParticipants < minParticipants) {
            errors.push(`Ticket ${index + 1}: maximum group participants must be greater than or equal to minimum`);
          }
        }
      });
    }
          ...
          if (!field.options || field.options.length === 0) {
            ...
            field.options.forEach((option, optionIndex) => {
              if (!option.label?.trim() || !option.value?.trim()) {
                errors.push(`Custom field ${index + 1}, option ${optionIndex + 1}: label and value are required`);
              }
            });
          }
        }
    ...
    if (stepId === 4 && eventDraft.allowCoupons) {
      eventDraft.coupons.forEach((coupon, index) => {
        ...
        if (!code) {
          ...
          errors.push(`Coupon ${index + 1}: code must be at least 5 uppercase alphanumeric characters`);
        }

        if ((coupon.discountValue ?? 0) <= 0) {
          errors.push(`Coupon ${index + 1}: discount value must be greater than 0`);
        }

        if (coupon.discountType === 'percentage' && (coupon.discountValue ?? 0) > 100) {
          errors.push(`Coupon ${index + 1}: percentage discount cannot exceed 100`);
        }
        ...
        if (coupon.startsAt && coupon.endsAt && new Date(coupon.startsAt) >= new Date(coupon.endsAt)) {
          errors.push(`Coupon ${index + 1}: valid until must be after valid from`);
        }
      });
    }
  ...
  return (
    <div className="container max-w-6xl mx-auto pb-6 space-y-6">
      {/* Header */}
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Create New Event</h1>
            <p className="text-muted-foreground">Set up your event in {steps.length} simple steps</p>
          </div>
        ...
        {steps.map((step) => {
          ...
          return (
            ...
            >
              ...
              >
                {isCompleted && !isCurrent ? <IconCheck className="w-5 h-5" /> : <Icon className="w-5 h-5" />}
              </div>
              <div className="text-center">
                <p className={cn('text-xs font-medium', isCurrent && 'text-primary')}>{step.title}</p>
              </div>
            </button>
          );
        })}
      ...
      <div className="flex items-center justify-between pt-6 border-t">
        ...
        <div className="flex gap-2">
          {currentStep === steps.length - 1 ? (
            ...
            <Button onClick={handleNext} className="gap-2" disabled={!canProceedToNext}>
              Next Step
