...
export function useSessionData(eventId) {
    ...
    const fetchMultipleSessionsStatus = useCallback(async () => {
        ...
        try {
            setError(null);
            const response = await api.get(`/events/admin/session/${eventId}/multiple-sessions-status`);
    ...
    const deleteSession = async (sessionId) => {
        try {
            setLoading(true);
            setError(null);
            const response = await api.delete(`/events/admin/session/${eventId}/${sessionId}`);
    ...
    const enableCheckIn = async (sessionId, checkInData) => {
        try {
            setLoading(true);
            setError(null);
            const response = await api.post(`/events/admin/checkin/${eventId}/${sessionId}`, checkInData);
    ...
    const disableCheckIn = async (sessionId) => {
        try {
            setLoading(true);
            setError(null);
            const response = await api.patch(`/events/admin/checkin/${eventId}/${sessionId}`);
            setSessions(prev =>
                prev.map(session =>
                    session._id === sessionId ? { ...session, ...response.data.session } : session
                )
