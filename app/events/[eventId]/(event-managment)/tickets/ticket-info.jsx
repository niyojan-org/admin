...
import { IconCircleCheck, IconPlus, IconTicket, IconTrendingUp, IconWallet } from '@tabler/icons-react';
...
function SummaryCard({ title, value, helper, icon: Icon, accentClass }) {
  return (
    <Card className="border-border/70 bg-background/80 shadow-sm backdrop-blur-sm px-2 py-2">
...
function TicketInfo({ tickets, handleCreateTicket, needCreateOption }) {
  const totalCapacity = tickets.reduce((sum, ticket) => sum + Number(ticket?.capacity || 0), 0);
  const totalSold = tickets.reduce((sum, ticket) => sum + Number(ticket?.sold || 0), 0);
      ...
      <CardContent className="">
        <div className="flex flex-col gap-1 lg:flex-row lg:items-end lg:justify-between">
          ...
          <p className="max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">
            Keep pricing, sales windows, and capacity easy to scan so you can make quick decisions without jumping
            between screens.
          </p>
          {needCreateOption && (
            <Button size="lg" className="h-11 rounded-full px-5" onClick={handleCreateTicket}>
        ...
        <div className="grid sm:gap-4 gap-2 grid-cols-2 xl:grid-cols-4 mt-2">
          <SummaryCard
            title="Ticket types"
          ...
          <SummaryCard
            ...
            helper={`${numberFormatter.format(Math.max(tickets.length - liveTickets, 0))} paused or inactive`}
