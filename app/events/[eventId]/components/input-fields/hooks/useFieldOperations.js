...
export function useFieldOperations(eventId, callbacks = {}) {
  ...
  const handleAddField = async () => {
    try {
      ...
      try {
        ...
        response = await api.post(`/events/admin/dynamic-field/${eventId}`, payload);
      } catch (newApiError) {
        // Fallback to old endpoint if new one doesn't exist
        response = await api.post(`/events/admin/${eventId}/add-input-field`, payload);
      }
  ...
  const handleEditField = async (fieldId) => {
    try {
      ...
      try {
        // Try new endpoint first (from documentation)
        response = await api.put(`/events/admin/dynamic-field/${eventId}/${fieldId}`, payload);
      } catch (newApiError) {
        console.log('New API failed, trying old endpoint:', newApiError.message);
        // Fallback to old endpoint if new one doesn't exist
        response = await api.put(`/events/admin/${eventId}/update-input-field/${fieldId}`, payload);
      }
  ...
  const handleDeleteField = async (fieldId) => {
    try {
      ...
      try {
        // Try new endpoint first (from documentation)
        response = await api.delete(`/events/admin/dynamic-field/${eventId}/${fieldId}`);
      } catch (newApiError) {
        console.log('New API failed, trying old endpoint:', newApiError.message);
        // Fallback to old endpoint if new one doesn't exist
        response = await api.delete(`/events/admin/${eventId}/delete-input-field/${fieldId}`);
      }
  ...
  const handleArrangeFields = async (order) => {
    try {
      ...
      try {
        // Try new endpoint first (from documentation)
        response = await api.post(`/events/admin/dynamic-field/${eventId}/arrange`, { order });
        console.log('New API response:', response.data);
      } catch (newApiError) {
        console.log('New API failed, trying old endpoint:', newApiError.message);
        // Fallback to old endpoint if new one doesn't exist
        response = await api.put(`/events/admin/${eventId}/arrange-input-fields`, { order });
