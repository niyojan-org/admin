...
function ManagementBanner() {
  ...
  const visibilityLabel =
    event.isPrivate || event.visibility === "private" ? "Private" : "Public";
  const modeLabel = event.mode ? toTitleCase(event.mode) : "Online";

  const registrationLabel = getRangeLabel(
  ...
  const eventDateLabel = getRangeLabel(
    sessionRange.start,
    sessionRange.end,
    formatDate,
  );
