...
export function useRegistrationManagement(eventId) {
  ...
  const fetchRegistrationData = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || "Failed to load registration data";
  ...
  const toggleRegistration = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || "Failed to toggle registration";
  ...
  const updateTimeline = useCallback(
    async (timeline) => {
      ...
      try {
        ...
        const errorMessage =
          err.response?.data?.message || "Failed to update timeline";
        toast.error(errorMessage);
        throw err;
      }
    },
  ...
  const extendDeadline = useCallback(
    async (extension) => {
      ...
      try {
        ...
        const errorMessage =
          err.response?.data?.message || "Failed to extend deadline";
        toast.error(errorMessage);
        throw err;
      }
    },
  ...
  const refresh = useCallback(() => {
    setIsLoading(true);
    return Promise.all([fetchRegistrationData(), fetchRequirements()]);
  ...
  const hasTimeline = !!(
    registrationData?.registrationStart && registrationData?.registrationEnd
  );
