    ...
    formattedData.sessions = formattedData.sessions.map((session) => {
      ...
      // If already in ISO datetime format (new format), ensure they are valid ISO strings
        ...
        try {
          ...
          formattedSession.startTime = new Date(
            session.startTime,
          ).toISOString();
          formattedSession.endTime = new Date(session.endTime).toISOString();
        } catch (error) {
          console.warn("Invalid datetime format in session:", error);
    ...
    if (session.startTime) {
      try {
        ...
        if (sessionStartTime <= regEndDate) {
          errors.push(
            `Session ${index + 1}: Session must start after registration end date (${regEndDate.toLocaleDateString()})`,
          );
        }
...
export const isSessionAfterRegistration = (
  sessionStartTime,
  registrationEndDate,
) => {
