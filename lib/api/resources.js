...
export async function uploadResource(file, type = "logo", title = "") {
    try {
        ...
        toast.error(error.response.data.message || "Failed to upload the resource.")
        console.error("Error uploading resource:", error);
        throw error;
    }
}

export async function uploadOrganizationLogo(file, organizationName = "Organization Logo") {
    return uploadResource(file, "logo", organizationName);
}

export async function uploadOrganizationCover(file, organizationName = "Organization Cover") {
    return uploadResource(file, "cover", organizationName);
}

export async function uploadEventBanner(file, eventName = "Event Banner") {
    return uploadResource(file, "banner", eventName);
}
