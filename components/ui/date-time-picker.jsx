...
export function DateTimePicker({
  ...
  const dislayFormat = useMemo(() => {
    ...
    const timeFormat = !hideTime 
      ? (use12HourFormat 
          ? (needSeconds ? ' hh:mm:ss a' : ' hh:mm a')
          : (needSeconds ? ' HH:mm:ss' : ' HH:mm'))
      : '';
    return format(
      displayValue,
      `${!hideTime ? 'MMM' : 'MMMM'} d, yyyy${timeFormat}`
    );
  }, [displayValue, hideTime, use12HourFormat, needSeconds]);
...
function TimePicker({
  ...
  const [second, setSecond] = useState(needSeconds ? value.getSeconds() : 0);
  ...
  const display = useMemo(() => {
    ...
    for (const element of ['hour', 'minute', 'second']) {
      if (element === 'second' && !needSeconds) continue;
  ...
  }, [value, use12HourFormat, timePicker, needSeconds]);
      ...
      <PopoverContent className="p-0" side="top">
        <div className="flex-col gap-2 p-2">
          <div className="flex h-56 grow">
            ...
            {needSeconds && (!timePicker || timePicker.second) && (
              <ScrollArea className="h-full grow">
                <div className="flex grow flex-col items-stretch overflow-y-auto pe-2 pb-48">
                  {seconds.map((v) => (
